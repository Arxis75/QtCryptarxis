...
class StrByteSetFormat
{
    public:
        ...
        inline uint8_t getByteSize() const { return m_aligned_byte_size; }
...
static const StrByteSetFormat Hex20("Hex", "0x", 16, 4, "^(0x)?[0-9a-fA-F]{0,40}",      20,  0); // Addresses
static const StrByteSetFormat Hex32("Hex", "0x", 16, 4, "^(0x)?[0-9a-fA-F]{0,64}",      32,  0); // Private Keys, Signature R, Signature S...
static const StrByteSetFormat Hex33("Hex", "0x", 16, 4, "^(0x)?0[2|3][0-9a-fA-F]{64}",  33,  0); // Compressed PubKeys
static const StrByteSetFormat Hex64("Hex", "0x", 16, 4, "^(0x)?[0-9a-fA-F]{0,128}",     64,  0); // BIP39 Seeds
